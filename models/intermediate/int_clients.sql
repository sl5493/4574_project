WITH clean_sessions AS (
    SELECT *
    FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY session_id ORDER BY session_at_ts) as rnk  FROM {{ref ('base_snowflake_sessions')}}) s
    WHERE rnk = 1
),
clean_orders AS (
    SELECT *
    FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY order_id ORDER BY order_at_ts) as rnk FROM  {{ref ('base_snowflake__ORDERS')}}) o
    WHERE rnk = 1
),
raw_client AS (
    SELECT 
...
SELECT client_id, client_name, phone, shipping_address, state, payment_info,
            ...
            session_count, viewed_item_count,
